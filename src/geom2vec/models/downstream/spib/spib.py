...
class _LobeEncoder(nn.Module):
    """Wrap a graph-based lobe so it can be used as a flat encoder in SPIB.

    Accepts:
    - dict or (graph_features, ca_coords) tuples, in which case the provided
      CA coordinates are used directly, or
    - plain graph feature tensors, for which synthetic coordinates are
      constructed in the same way as the VAMP workflow.
    """

    def __init__(self, base_lobe: nn.Module, num_tokens: int, hidden_channels: int) -> None:
        super().__init__()
        self.base_lobe = base_lobe
        self.num_tokens = num_tokens
        self.hidden_channels = hidden_channels

    def forward(self, x: torch.Tensor | dict | tuple) -> torch.Tensor:
        # Normalize inputs to (graph_features, optional ca_coords).
        ca_coords = None
        if isinstance(x, dict):
            if "graph_features" not in x:
                raise ValueError("Input dict must contain 'graph_features'.")
            graph_features = x["graph_features"]
            ca_coords = x.get("ca_coords")
        elif isinstance(x, (tuple, list)):
            if len(x) != 2:
                raise ValueError(
                    "Tuple/list inputs to _LobeEncoder must be (graph_features, ca_coords)."
                )
            graph_features, ca_coords = x
        else:
            graph_features = x
        ...
        if graph_features.dim() == 2:
            ...
            raise ValueError("_LobeEncoder expects 2D or 4D graph feature inputs.")

        if ca_coords is None:
            # Try to build default CA coordinates in the same way as VAMPNet.
            coords = _default_ca_coords_from_features(
                graph_features.detach().cpu(), dtype=graph_features.dtype
            )
            if coords is None:
                raise RuntimeError("Failed to construct default CA coordinates from features.")
            ca_coords = coords.to(device=graph_features.device, dtype=graph_features.dtype)
        else:
            if not isinstance(ca_coords, torch.Tensor):
                ca_coords = torch.as_tensor(ca_coords, dtype=graph_features.dtype, device=graph_features.device)
            if ca_coords.dim() == 2:
                ca_coords = ca_coords.unsqueeze(0).expand(batch, -1, -1)
            if ca_coords.shape[0] != batch or ca_coords.shape[1] != self.num_tokens or ca_coords.shape[2] != 3:
                raise ValueError(
                    "ca_coords must have shape (batch, num_tokens, 3) compatible with graph_features."
                )

        return self.base_lobe((graph_features, ca_coords))
...
class SPIBModel(SPIBVAE):
    ...
    def from_lobe(
        ...
        *,
        ...
        lobe = lobe.to(torch_device)

        # Infer encoder output dimension from the lobe's output projection.
        encoder_dim: int | None = None
        output_proj = getattr(lobe, "output_projection", None)
        if output_proj is not None and hasattr(output_proj, "layers"):
            for layer in reversed(output_proj.layers):
                if isinstance(layer, nn.Linear):
                    encoder_dim = layer.out_features
                    break
        if encoder_dim is None:
            raise ValueError("Could not infer encoder output dimension from lobe.")

        encoder = _LobeEncoder(lobe, num_tokens=num_tokens, hidden_channels=hidden_channels).to(torch_device)

        model = cls(
            output_dim=output_dim,
            data_shape=(num_tokens, 4, hidden_channels),
            z_dim=z_dim,
            lagtime=lag_time,
            beta=beta,
    ...
    def fit(
        ...
        index: int = 0,
        max_epochs_per_refinement: int | None = None,
    ):
        ...
        while True:
            ...
            unchanged_epochs = self._update_unchanged_epochs(
                train_losses["loss"], train_epoch_loss_prev, tolerance, unchanged_epochs
            )
            train_epoch_loss_prev = train_losses['loss']

            reached_patience = unchanged_epochs > patience
            reached_max_epochs = max_epochs_per_refinement is not None and epoch >= max_epochs_per_refinement

            if reached_patience or reached_max_epochs:
